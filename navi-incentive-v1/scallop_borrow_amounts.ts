...
async function borrowTest(amount: number): Promise<void> {
  ...
  console.log(`borrow USDC amount=${amount}: ${status} → ${error ? (error.includes("MoveAbort") ? parseAbort(error) : error.slice(0, 80)) : "ok"}`);
}

async function main() {
  console.log("=== Borrow USDC with varying amounts ===\n");
  await borrowTest(0);
  await borrowTest(1);
  await borrowTest(100);
  await borrowTest(1_000_000);  // 1 USDC (6 decimals)
  await borrowTest(10_000_000); // 10 USDC
}
